...
numbx = open('numx.txt','r').read().splitlines()
numxz = random.choice(numbx)
...
def banner():
    os.system("clear")
    print("FACEBOOK AUTO ID CREATOR")
    print(f"{W}———————————————————————————————")
    print(f"{R}1{W} CREATE WITH PANEL NUMBER")
    print(f"{R}2{W} CREATE WITH GMAIL")
    print(f"{R}3{W} CREATE WITH TEMP MAIL")
    select = input("~>>")
    if "1" in select:
     main3()
    if "2" in select:
...
def main() -> None:
    ...
    for make in range(100):
        ...
        print(f"   {M} NAME  - {G}{firstname} {lastname}")
        print(f"   {M} TEMP-MAIL - {G}{email2}")
        print(f"   {M} PASS - {G} ahmed11")
        payload = {
            'ccp': "2",
            ...
            'guid': "",
            'pre_form_step': "",
            'encpass': '#PWD_BROWSER:0:{}:{}'.format(str(time.time()).split('.')[0],"ahmed11"),
            'submit': "Sign Up",
        ...
        if "c_user" in py_submit.cookies:
            ...
            if valid:
                print(f"   {M} FB UID - {G}{uid}")
                res = requests.get(f"https://rajx.pythonanywhere.com/live?uid={uid}")
                print(res.text)
                print(f"   {M} LOGIN OTP - {G}{valid}")
                confirm_id(email2,uid,valid,con_sub,ses)
            else:
                print(f"{K} \x1b[38;5;206m DISABLED ")
                linex()
        else:
            print(f"{K} {R} CHECKPOINT ")
            linex()
def confirm_id(mail,uid,otp,data,ses):
    try:
        ...
        if "checkpoint" in str(response.url):
            print(f"{K}{R} DISABLED")
            linex()
        else:
            cookie = (";").join([ "%s=%s" % (key,value) for key,value in ses.cookies.get_dict().items()])
            print(f"   {M} SUCCESS - {G}{uid}|ahmed11|{cookie}")
            open("/sdcard/SUCCESS-OK-ID.txt","a").write(uid+"|MrCode@123/ahmed11|"+cookie+"\n")
            linex()
    except Exception as e:
        linex()
        pass
...
def main_two():
    ...
    for make in range(100):
        ...
        #email2 = GetEmail()
        email2 = input("GMAIL->") 
        #email2 = random.choice(numbx)
        firstname,lastname = fake_name()
        print(f"     {M} NAME  - {W}{firstname} {lastname}")
        print(f"     {M} GMAIL - {W}{email2}")
        print(f"     {M} PASS - {W} ahmed11")
        payload = {
            'ccp': "2",
            'reg_instance': str(formula["reg_instance"]),
            'submission_request': "true",
...
def main3():
    ...
    for make in range(100):
        ...
        formula = extractor(response.text)
        #email2 = GetEmail()
        email2 = input(f"{X} NUMBER->") 
        #email2 = random.choice(numbx)
        firstname,lastname = fake_name()
        print(f"        {M} NAME  - {W}{firstname} {lastname}")
        print(f"        {M} NUMBER - {W}{email2}")
        print(f"        {M} PASS - {W} ahmed11")
        payload = {
            'ccp': "2",
            'reg_instance': str(formula["reg_instance"]),
